...
```
save_script(domain, run_id, script_type="chapter_list",   test_url=<目录页 URL>, ocr=false) -> 第一次，只写 chapter_list_js
save_script(domain, run_id, script_type="chapter_content", test_url=<章节页 URL>, ocr=true)  -> 第二次，只写 chapter_content_js + ocr=1
```
...
2. 调用 save_script 落库（两次）：
   - save_script(domain, run_id, script_type="chapter_list",    test_url=<目录页>, ocr=<true|false>)
   - save_script(domain, run_id, script_type="chapter_content", test_url=<章节页>, ocr=<同上>)
3. save_script 返回 success=false -> 按 diagnostic/suggestion 修 JS，重新 execute_js 调试，再 save_script
   （注意：两次调用要分别验证通过，落库前都会跑一次试运行）
...
已有 23 个 agent 工具、`WebViewExtractScenario` 现有 happy path、两个 Headless service 现有流程都不能破。`save_script` 重写后参数签名变更：移除 `list_run_id`/`content_run_id`/`chapter_list_js`/`chapter_content_js`/`url_pattern`，新增 `run_id`/`script_type`/`test_url`/`ocr`（全必填）。所有引用 `save_script` 的地方（仅 `WebViewExtractScenario` 自身）同步更新。
