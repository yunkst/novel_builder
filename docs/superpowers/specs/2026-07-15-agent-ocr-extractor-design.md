...
- `ToolArgParser.optionalBool('ocr')` 解析
  - 实现计划任务第一步：`grep -r "optionalBool" lib/services/novel_agent/` 确认存在
  - 存在 → 直接用；不存在 → 在 `tool_executor_helpers.dart` 新增（参考 `nullableString` 范式：`(bool?, String?) optionalBool(String key)`）
- 未传时默认 false
