...
class NovelSyncData(BaseModel):
    ...
    character_relations: list[CharacterRelationSyncData] = Field(
        default=[], description="角色关系列表"
    )
    outlines: list[OutlineSyncData] = Field(default=[], description="大纲列表")
...
class NovelSyncDownloadResponse(BaseModel):
    ...
    success: bool = Field(..., description="是否成功")
    message: str = Field(..., description="响应消息")
    novel_data: NovelSyncData | None = Field(None, description="小说数据")
    sync_version: int = Field(..., description="同步版本号")
    synced_at: str = Field(..., description="同步时间(ISO格式)")
...
class NovelSyncListResponse(BaseModel):
    ...
    success: bool = Field(..., description="是否成功")
    message: str = Field(..., description="响应消息")
    novels: list[NovelSyncData] = Field(default=[], description="小说列表")
    total_count: int = Field(..., description="总数")
    page: int = Field(1, description="当前页码")
    page_size: int = Field(20, description="每页数量")


class NovelSyncDeleteResponse(BaseModel):
    """小说同步删除响应模式."""
