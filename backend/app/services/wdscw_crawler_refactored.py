...
class WdscwCrawlerRefactored(BaseCrawler):
    ...
    async def search_novels(self, keyword: str) -> list[dict[str, Any]]:
        ...
        try:
            ...
            response = await self.get_page(
                search_url, custom_headers=self.custom_headers, timeout=30
            )
            ...
            if result_container:
                # 查找所有小说链接
                novel_links = result_container.find_all(
                    "a", href=re.compile(r"www\.5dscw\.com/book_\d+")
                )
                ...
                for link in novel_links:
                    ...
                    if title and href and len(title) > 2:
                        # 尝试获取作者信息
                        author = ""
                        author_elem = link.find_next(
                            "span", string=re.compile(r"著|作者")
                        )
                        if author_elem:
                            author = (
    ...
    async def get_chapter_list(self, novel_url: str) -> list[dict[str, Any]]:
        ...
        try:
                    ...
                    for container in containers_to_check:
                        if container:
                            links = container.find_all(
                                "a", href=re.compile(r"/book_\d+/\d+\.html")
                            )
                            if (
                                len(links) > 20
                            ):  # 如果找到的链接数量较多，认为是完整章节列表
                                chapter_links = links
                                break

                    # 如果在容器中没找到足够多的链接，尝试查找后续兄弟元素
                    if len(chapter_links) <= 20:
                        chapter_links = complete_section.find_all_next_siblings(
                            "a", href=re.compile(r"/book_\d+/\d+\.html")
                        )
            ...
            if len(chapter_links) <= 20:
                all_chapter_links = soup.find_all(
                    "a", href=re.compile(r"/book_\d+/\d+\.html")
                )
                ...
                for link in all_chapter_links:
                    ...
                    if (
                        ...
                        and not any(
                            skip in title.lower()
                            for skip in ["首页", "书库", "分类", "排行", "登录", "注册"]
                        )
                        and re.match(r"/book_\d+/\d+\.html", href)
                    ):
            ...
            for i, link in enumerate(chapter_links):
                ...
                if chapter_title and chapter_url:
                    chapters.append(
                        {"title": chapter_title, "url": chapter_url, "index": i + 1}
                    )
    ...
    async def get_chapter_content(self, chapter_url: str) -> dict[str, Any]:
        ...
        try:
            ...
            while current_url:
                response = await self.get_page(
                    current_url, custom_headers=self.custom_headers, timeout=30
                )
                if response.status_code != 200:
                    break
                ...
                next_page_elem = soup.find(
                    "a", string=re.compile(r"下一页|下页|下一頁")
                )
                if next_page_elem:
                ...
                if not next_page_link:
                    ...
                    page_pattern = re.compile(
                        rf"{re.escape(current_chapter_id)}_(\d+)\.html$"
                    )
                    ...
                    for link in all_links:
                        ...
                        if match:
                            ...
                            if found_page_num > page_num:
                                if href.startswith("/"):
                                    next_page_link = urllib.parse.urljoin(
                                        self.base_url, href
                                    )
                                elif href.startswith("http"):
                                    next_page_link = href
                ...
                if next_page_link:
                    # 检查是否还在同一章节
                    next_chapter_id = (
                        next_page_link.split("/")[-1].split(".")[0].split("_")[0]
                    )
                    if (
                        next_chapter_id != current_chapter_id
                        or next_page_link == current_url
                    ):
                        next_page_link = None
    ...
    def _extract_chapter_content(self, soup) -> str:
        ...
        if not content_div:
            ...
            for div in divs:
                ...
                if div.get("class") and any(
                    cls in ["nav", "menu", "header", "footer", "sidebar"]
        ...
        if content_div:
            ...
            for ad in content_div.find_all(
                ["script", "style", "ins", "iframe", "noscript", "form"]
            ):
                ad.decompose()
        ...
        if content:
            # 移除常见的广告词汇和无用文本
            content = re.sub(
                r"·[\d.]*\\?[\\\/]?[小小说说]?网[′\'`]?_?[无无]?错?内?容?\.?",
                "",
                content,
            )
            content = re.sub(
                r"[\\\/]0\.[0-9]*\\?[\\\/]?[小小说说]?网[′\'`]?_?[无无]?错?内?容?\.?",
                "",
                content,
            )
            content = re.sub(
                r"(?:温馨提示|热门推荐|本章未完|请点击下一页).*$",
                "",
                content,
                flags=re.MULTILINE,
            )
