...
class SmxkuCrawler(BaseCrawler):
    ...
    async def search_novels(self, keyword: str) -> list[dict[str, Any]]:
        ...
        try:
            ...
            config = RequestConfig(
                timeout=15, max_retries=3, strategy=RequestStrategy.HYBRID
            )

            # 发送搜索请求
            response = await self.http_client.get(
                f"{search_url}?searchkey={keyword}", config
            )
            ...
            for h4 in novel_containers:
                ...
                description = ""
                if parent_element:
                    desc_elem = parent_element.find(
                        ["p", "div"], class_=re.compile(r"intro|desc|content")
                    )
                    if desc_elem:
    ...
    async def get_chapter_list(self, novel_url: str) -> list[dict[str, Any]]:
        ...
        try:
            ...
            # 配置请求
            RequestConfig(timeout=15, max_retries=3, strategy=RequestStrategy.HYBRID)
            ...
            if not chapter_list:
                ...
                chapter_links = soup.find_all(
                    "a", href=re.compile(f"^/{novel_id}/\\d+\\.html$")
                )
                ...
                # 从容器中提取章节
                chapter_links = chapter_list.find_all(
                    "a", href=re.compile(f"^/{novel_id}/\\d+\\.html$")
                )
    ...
    async def _extract_novel_from_detail_page(
        self, soup, novel_url: str, novel_id: str
    ) -> dict[str, Any]:
        """从小说详情页提取小说信息"""
        try:
            ...
            title = (
                title_elem.get_text(strip=True) if title_elem else f"小说ID: {novel_id}"
            )
            ...
            for pattern in author_patterns:
                if pattern:
                    if pattern.name == "a":
                        ...
                        author_link = (
                            pattern.find_next("a") if pattern.name != "a" else pattern
                        )
                        if author_link:
                            author = author_link.get_text(strip=True)
