...
class ComfyUIVideoClient:
    ...
    def replace_image_placeholder(self, workflow_data: Dict[str, Any], image_base64: str) -> Dict[str, Any]:
        """替换工作流中的图片占位符.
            ...
            for node_id, node_data in updated_workflow.items():
                ...
                for key, value in inputs.items():
                    if isinstance(value, str) and "图片base64在这里替换" in value:
                        # 替换占位符
                        new_value = value.replace("图片base64在这里替换", image_base64)
                        inputs[key] = new_value
                        logger.info(f"在节点 {node_id} 的字段 {key} 中替换了图片占位符")
    ...
    async def generate_video_from_image(self, video_prompt: str, image_base64: str) -> Optional[str]:
        """从图片生成视频.
        ...
        try:
            ...
            # 替换图片占位符
            workflow_data = self.replace_image_placeholder(workflow_data, image_base64)

            # 设置视频提示词
            workflow_data = self._set_video_prompt(workflow_data, video_prompt)

            # 设置随机种子
            workflow_data = self._set_random_seed(workflow_data)
    ...
    def _set_random_seed(self, workflow_data: Dict[str, Any], seed: int = None) -> Dict[str, Any]:
        """设置随机种子.
    ...
    async def wait_for_video_completion(self, task_id: str, timeout: int = 600) -> Optional[str]:
        """等待视频生成完成并获取视频文件名.
                ...
                for node_id, node_output in outputs.items():
                    ...
                    if "images" in node_output:
                        for image in node_output["images"]:
                            ...
                            if filename and filename.lower().endswith(('.gif', '.mp4', '.webm')):
                                logger.info(f"找到生成的视频文件: {filename}")
                                return filename
    ...
    async def get_video_data(self, filename: str) -> Optional[bytes]:
        """获取视频二进制数据.
...
def create_comfyui_video_client() -> ComfyUIVideoClient:
    ...
    workflow_path = os.getenv("COMFYUI_VIDEO_WORKFLOW_PATH", "./comfyui_json/image2video/image_to_video.json")
