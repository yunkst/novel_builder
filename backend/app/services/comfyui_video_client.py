...
class ComfyUIVideoClient:
    ...
    def _load_workflow(self) -> None:
        ...
        try:
            ...
            if not workflow_file.exists():
                raise FileNotFoundError(f"图生视频工作流文件不存在: {self.workflow_path}")
    ...
    def replace_image_placeholder(self, workflow_data: dict[str, Any], image_filename: str) -> dict[str, Any]:
        """替换工作流中的图片占位符.
            ...
            for node_id, node_data in updated_workflow.items():
                ...
                for key, value in inputs.items():
                    if isinstance(value, str) and "图片base64在这里替换" in value:
                        # 替换占位符为文件名
                        new_value = value.replace("图片base64在这里替换", image_filename)
                        inputs[key] = new_value
                        logger.info(f"在节点 {node_id} 的字段 {key} 中替换了图片占位符为文件名: {image_filename}")
    ...
    async def generate_video_from_image(self, video_prompt: str, image_filename: str) -> str | None:
        """从图片生成视频.
        ...
        try:
            ...
            response = requests.post(upload_url, files={'image': (image_filename, response.content, 'image/png')}, timeout=30)

            # 准备工作流数据
            workflow_data = open(self.workflow_path, encoding='utf-8').read()

            # 替换图片占位符
            workflow_data = workflow_data.replace("图片base64在这里替换", image_filename)

            # 设置视频提示词
            workflow_data = workflow_data.replace("提示词在这里替换", video_prompt)

            # 设置随机种子
            workflow_data = workflow_data.replace('"在这替换随机数"', str(random.randint(1, 999999999)))
            ...
            if response.status_code == 200:
                ...
                logger.error(f"ComfyUI图生视频API请求失败: {response.status_code} - {response.text}")
                return None
    ...
    def _set_video_prompt(self, workflow_data: dict[str, Any], video_prompt: str) -> dict[str, Any]:
        """设置视频提示词.
    ...
    def _set_random_seed(self, workflow_data: dict[str, Any], seed: int | None = None) -> dict[str, Any]:
        """设置随机种子.
    ...
    async def wait_for_video_completion(self, task_id: str, timeout: int = 3600) -> str | None:
        """等待视频生成完成并获取视频文件名.
                ...
                for node_output in outputs.values():
                    ...
                    if "images" in node_output:
                        for image in node_output["images"]:
                            ...
                            if filename and (filename.lower().endswith(('.gif', '.mp4', '.webm')) or subfolder == "video"):
                                logger.info(f"找到生成的视频文件: {filename}, subfolder: {subfolder}")
                                # 返回格式化的文件路径
                                if subfolder:
    ...
    async def get_video_data(self, filename: str, subfolder: str | None = None) -> bytes | None:
        """获取视频二进制数据.
        ...
        try:
            ...
            if response.status_code == 200:
                logger.info(f"✅ 成功获取视频: {filename}, 大小: {len(response.content)} bytes")
