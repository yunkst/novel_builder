...
class ShukugeCrawler(BaseCrawler):
    ...
    def _extract_novels(self, soup: BeautifulSoup, keyword: str) -> list[dict]:
        ...
        for link in all_links:
            ...
            href = link.get("href", "")
            if keyword.lower() in title.lower() and any(
    ...
    def get_chapter_list(self, novel_url: str) -> list[dict]:
        try:
            ...
            def _extract_chapters_from_soup(
                any_soup: BeautifulSoup, base: str
            ) -> list[dict]:
                results: list[dict] = []
                # 优先在常见容器中提取
                container = (
                    any_soup.find("div", id="list")
                    or any_soup.find("div", class_="listmain")
                    or any_soup.find("dl")
                    or any_soup.find("div", class_="book_list")
                    or any_soup.find("ul", class_="chapterlist")
                    or any_soup.find("div", id="readerlist")
                    or any_soup.find(
                        "div", class_=re.compile(r"list|chapter|content", re.I)
                    )
                )
                skip_words = [
                    "封面",
                ...
                for a in any_soup.find_all("a", href=True):
                    href = a.get("href", "")
                    title = a.get_text().strip()
                    is_candidate_href = bool(
                        re.search(
                            r"/\d+\.html$|/book/\d+/?|/read/\d+|/chapters?/\d+", href
                        )
                    )
                    ...
                    if (
                        is_candidate_href
                        and len(title) > 1
                        and looks_like_chapter
                        and not any(sw in title for sw in skip_words)
                    ):
                        results.append(
                            {"title": title, "url": urllib.parse.urljoin(base, href)}
                        )
