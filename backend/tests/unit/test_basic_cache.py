    ...
    def test_api_endpoint_structure(self):
        ...
        for endpoint in endpoints:
            assert endpoint.startswith("/api/"), (
                f"Endpoint {endpoint} should start with /api/"
            )
            assert len(endpoint) > 5, (
                f"Endpoint {endpoint} should have reasonable length"
            )

    def test_error_response_format(self):
        """测试错误响应格式"""
        # Given - 错误响应示例
        error_response = {"error": True, "message": "Test error message", "code": 400}
