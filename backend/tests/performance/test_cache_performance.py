...
class TestCachePerformance:
    ...
    async def api_client(self):
        ...
        return aiohttp.ClientSession(
            timeout=aiohttp.ClientTimeout(total=60, connect=5), connector=connector
        )
            ...
            for _ in range(10):  # 每个端点测试10次
                ...
                if endpoint == "health":
                    ...
                    async with api_client.get(
                        f"{base_url}/{endpoint}", headers=valid_headers
                    ) as response:
    ...
    @pytest.mark.performance
    async def test_concurrent_cache_requests_performance(
        self, api_client, valid_headers, base_url
    ):
        """测试并发缓存请求性能"""
        # Given
        ...
        for level in concurrent_levels:
            ...
            async def create_cache_task(task_index):
                try:
                    create_data = {
                        "novel_url": f"https://example.com/novel/concurrent-{level}-{task_index}"
                    }
                    async with api_client.post(
        ...
        for level, result in results.items():
            ...
            assert result["success_rate"] > 0.95, (
                f"并发级别 {level} 的成功率过低: {result['success_rate']}"
            )
            ...
            if level > 10:
                ...
                assert time_ratio < 1.5, (
                    f"并发 {level} vs {level // 2 * 2} 时间增长不合理: {time_ratio:.2f}x"
                )
    ...
    @pytest.mark.performance
    async def test_large_data_handling_performance(
        self, api_client, valid_headers, base_url
    ):
        """测试大数据处理性能"""
        # Given - 模拟大量缓存任务和章节
        ...
        assert creation_time < 60, (
            f"创建 {large_task_count} 个任务耗时过长: {creation_time:.2f}s"
        )
        assert len(task_ids) > large_task_count * 0.8, (
            f"任务创建成功率过低: {len(task_ids)}/{large_task_count}"
        )
        ...
        async with api_client.get(
            f"{base_url}/api/cache/tasks?limit={large_task_count}",
            headers=valid_headers,
        ) as response:
            assert response.status == 200
            result = await response.json()
        ...
        assert list_time < 5, (
            f"获取 {large_task_count} 个任务列表耗时过长: {list_time:.2f}s"
        )
    ...
    async def test_memory_usage_monitoring(self, api_client, valid_headers, base_url):
        ...
        for batch in range(5):  # 5个批次，每批20个任务
            ...
            for i in range(20):
                create_data = {
                    "novel_url": f"https://example.com/novel/memory-test-{batch}-{i}"
                }
                tasks.append(
                    api_client.post(
                        f"{base_url}/api/cache/create",
    ...
    async def test_websocket_performance(self, api_client, valid_headers, base_url):
        ...
        try:
            ...
            async with api_client.ws_connect(ws_url) as ws:
                ...
                assert connection_time < 1.0, (
                    f"WebSocket连接时间过长: {connection_time:.3f}s"
                )
        ...
        if message_times:
            ...
            print(
                f"  消息时间范围: {min(message_times):.3f}s - {max(message_times):.3f}s"
            )

    @pytest.mark.stress
    async def test_sustained_load_stress_test(
        self, api_client, valid_headers, base_url
    ):
        """测试持续负载压力测试"""
        # Given
        ...
        async def sustained_requests():
            while time.time() - start_time < duration:
                ...
                create_data = {
                    "novel_url": f"https://example.com/novel/stress-test-{int(time.time())}"
                }
        ...
        # 验证持续性能
        assert actual_rps >= rps_target * 0.8, (
            f"实际RPS {actual_rps:.1f} 低于目标 {rps_target} 的80%"
        )
        assert success_count / total_requests_processed > 0.95, (
            f"成功率过低: {success_count / total_requests_processed:.1%}"
        )
        ...
        if response_times:
            ...
            assert avg_response_time < 2.0, (
                f"平均响应时间过长: {avg_response_time:.3f}s"
            )
    ...
    def test_system_resource_limits(self):
        ...
        print(
            f"  内存使用率: {(1 - memory_info.available / memory_info.total) * 100:.1f}%"
        )
        ...
        # Then - 验证进程资源使用在合理范围
        assert process_memory.rss / 1024 / 1024 < 512, (
            "进程内存使用过高: {process_memory.rss/1024/1024:.1f} MB"
        )
    ...
    @pytest.mark.performance
    async def test_cache_service_resource_cleanup(
        self, api_client, valid_headers, base_url
    ):
        """测试缓存服务资源清理"""
        # Given - 监控初始资源
