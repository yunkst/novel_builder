#!/usr/bin/env python3
"""
Syshangguan Novel Website Crawler - Working Version
工作版本：爱尚小说网 (https://m.syshangguan.com/)
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('working_syshangguan.log', encoding='utf-8', errors='replace'),
...
class SyshangguanCrawler:
    def __init__(self):
        ...
        self.session.headers.update({
            ...
            'Accept-Encoding': 'gzip, deflate',  # 不包含br，避免brotli压缩
    ...
    def decompress_content(self, response):
        ...
        if content_encoding == 'br':
            ...
            try:
                ...
                self.log_warning(f"Brotli解压失败: {str(e)}")
            ...
            try:
                ...
                self.log_warning(f"Gzip解压失败: {str(e)}")
                return response.text
        else:
            # 无压缩
            return response.text
    ...
    def get_novel_info(self, novel_url):
        ...
        try:
            ...
            # 方法1：查找包含"作者"的文本
            author_found = False
            for element in soup.find_all(text=re.compile(r'作者：')):
                if element and '作者：' in element:
                    author = element.replace('作者：', '').strip()
                    author_found = True
                    break
    ...
    def get_novel_chapters(self, novel_url, novel_id):
        ...
        try:
            ...
            # 处理压缩内容
            index_html_content = self.decompress_content(index_response)
    ...
    def _parse_chapter_list(self, soup, novel_id):
        ...
        # 如果没找到，尝试查找所有链接
        if not chapters:
            self.log_info("方法1未找到章节，尝试查找所有链接")
            ...
            for link in all_links:
                ...
                # 匹配章节URL模式
                if re.match(rf'/{novel_id}/\d+\.html', href) and title and len(title) > 1:
                    chapter_url = urljoin(self.base_url, href)
                    chapter_id = href.split('/')[-1].replace('.html', '')
