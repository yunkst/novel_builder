...
**个人技能存放在智能体特定的目录中（Codex 用 `~/.Codex/skills`，Codex 用 `~/.agents/skills/`）**
...
**技能**是经过验证的技术、模式或工具的参考指南。技能帮助未来的 Codex 实例找到并应用有效的方法。
...
**发现至关重要：** 未来的 Codex 需要找到你的技能

### 1. 丰富的描述字段

**目的：** Codex 读取描述来决定为当前任务加载哪些技能。让它能回答："我现在应该读这个技能吗？"
...
**为什么这很重要：** 测试表明，当描述总结了技能的工作流时，Codex 可能会跟随描述而非阅读完整的技能内容。一个写着"任务间进行代码审查"的描述导致 Codex 只做了一次审查，尽管技能的流程图清楚地展示了两次审查（先规格合规再代码质量）。

当描述改为仅"在当前会话中执行包含独立任务的实现计划时使用"（无工作流摘要）时，Codex 正确地阅读了流程图并遵循了两阶段审查流程。

**陷阱：** 总结工作流的描述创建了 Codex 会走的捷径。技能正文变成了 Codex 跳过的文档。

```yaml
# 错误：总结了工作流 - Codex 可能会跟随描述而非阅读技能
