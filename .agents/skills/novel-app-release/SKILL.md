...
此技能用于自动化 Novel Flutter 应用的发布流程。**由 Codex 执行时,changelog 由 AI 分析代码变更后撰写**(高质量、语义化);脚本本身保留规则生成作为 CI / 纯手动运行的兜底。
...
3. **生成更新日志** — Codex 分析 git diff/log,撰写面向用户的 changelog
...
**这是本次发布用户能看到的核心信息,必须由你(Codex)分析代码后撰写,不要依赖脚本的机械生成。**
...
EOF
)" python .Codex/skills/novel-app-release/scripts/build_and_upload.py
```
...
> ⚠️ 兜底输出是**文件/模块级**的机械摘要(如"优化服务层(5 个文件)"),语义质量有限。**Codex 执行时应始终走第二步(AI 生成),不要依赖兜底。**
...
```bash
SKIP_PREFLIGHT=1 python .Codex/skills/novel-app-release/scripts/build_and_upload.py
```
...
```
.Codex/skills/novel-app-release/
├── SKILL.md                    # 本文档
├── FIXES.md                    # 修复记录
└── scripts/
...
- **2026-06-22**: changelog 生成改为 **AI 驱动**:SKILL.md 新增「第二步:生成更新日志」专章,指导 Codex 分析 git diff/log 撰写语义化 changelog 并通过 `CHANGELOG` 环境变量注入脚本;脚本规则生成降级为兜底机制(CI / 纯手动运行时);明确职责分工
